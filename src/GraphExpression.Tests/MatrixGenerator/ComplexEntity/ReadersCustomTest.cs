...
namespace GraphExpression.Tests
{
    public class ReadersCustomTest
    {
        ...
        {
            public IEnumerable<ComplexEntity> GetMembers(ComplexBuilder builder, GraphExpression.Expression<object> expression, object entity)
            {
                if (entity is Test)
                {
                    var method = entity
        ...
        public void CreateCustomSerialize_ReturnExpressionAsString()
        {
            ...
            var expected = $"{{{test.GetHashCode()}}}" + " + {[0]: \"value1\"} + {@Capacity: 4} + {@Count: 1} + {HelloWorld(value1,value2): \"value1-value2\"}";
