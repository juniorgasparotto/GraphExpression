...
namespace GraphExpression.Tests
{
    public class DictionaryTest
    {
        ...
        public void CreateDictionary_ReturnExpressionAsString()
        {
            ...
            var expected = $"{{{dic.GetHashCode()}}} + ({{[0].{key1}}} + {{@Key: \"key1\"}} + {{@Value: \"value1\"}}) + ({{[1].{key2}}} + {{@Key: \"key2\"}} + {{@Value: \"value2\"}}) + ({{[2].{key3}}} + {{@Key: \"key3\"}} + {{@Value: \"value3\"}}) + {{@Comparer.{dic.Comparer.GetHashCode()}}} + {{@Count: 3}}";
